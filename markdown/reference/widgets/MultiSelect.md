...
* **`visual_size`** (str, default="medium"): Controls the visual density of the multi-select control (distinct from `size`, which controls the number of visible option rows). One of "small", "medium", or "large".
...
The `visual_size` parameter controls the visual density/compactness of the widget. This is distinct from the `size` parameter demonstrated above, which controls the number of visible option rows:


```python
pmui.FlexBox(
    *(pmui.MultiSelect(label=visual_size, options=['A', 'B', 'C'], value=['A'], visual_size=visual_size) for visual_size in pmui.MultiSelect.param.visual_size.objects)
)
```
