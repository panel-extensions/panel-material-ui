...
- **`size`** (str): Controls the visual size/density of the time picker input field. One of "small", "medium", or "large". Default is "medium".
...
```python
pmui.TimePicker(label='Meeting Time', helper_text='Choose a time slot')
```
...
The `size` parameter allows you to control the visual size and density of the `TimePicker` input field. Choose from "small", "medium" (default), or "large":


```python
pmui.FlexBox(
    *[pmui.TimePicker(label=size, size=size) for size in pmui.TimePicker.param.size.objects]
)
```
