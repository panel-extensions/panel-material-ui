...
- **`size`** (str, default="medium"): Controls the visual size/density of the date and time picker input field. One of "small", "medium", or "large".
...
The `size` parameter allows you to control the visual size and density of the `DatetimePicker` input field. Choose from "small", "medium" (default), or "large":


```python
pmui.Row(
    *[pmui.DatetimePicker(label=size, value=dt.datetime(2024, 4, 14, 10, 30), size=size) for size in pmui.DatetimePicker.param.size.objects],
    height=400
)
```
