...
- **`visual_size`** (str, default="medium"): Controls the visual size/density of the cross-selector control. One of "small", "medium", or "large".
...
Customize the visual size/density of the `CrossSelector` widget using the `visual_size` parameter:


```python
pn.FlexBox(*[pmui.CrossSelector(
    label=visual_size, value=['Apple', 'Pear'], options=['Apple', 'Banana', 'Pear', 'Strawberry'], visual_size=visual_size
) for visual_size in pmui.CrossSelector.param.visual_size.objects])
```
