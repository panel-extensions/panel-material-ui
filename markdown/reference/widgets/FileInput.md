...
* **`uploaded_label`** (str): Label shown on the button after a file is uploaded. Supports `{filename}` and `{n}` placeholders. Defaults to `None`, which displays `'Uploaded <filename>'`.
...
### Customizing the Uploaded Label

By default, after a file is selected the button label changes to `'Uploaded <filename>'`. Use `uploaded_label` to control that text. It supports two optional placeholders:

- `{filename}` - replaced with the uploaded filename (or comma-separated names for multiple files)
- `{n}` - replaced with the number of uploaded files

A static string keeps the button at a predictable width, while the placeholders let you include file details when useful.


```python
pmui.FlexBox(
    pmui.FileInput(label="Upload", uploaded_label="Uploaded"),
    pmui.FileInput(label="Upload", uploaded_label="{filename}"),
    pmui.FileInput(label="Upload files", uploaded_label="{n} file(s) ready", multiple=True),
)
```
