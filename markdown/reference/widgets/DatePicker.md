...
- **`size`** (str, default="medium"): Controls the visual size/density of the date picker input field. One of "small", "medium", or "large".
...
The `size` parameter allows you to control the visual size and density of the `DatePicker` input field. Choose from "small", "medium" (default), or "large":


```python
pmui.FlexBox(
    *[pmui.DatePicker(label=size, value="2024-04-01", size=size) for size in pmui.DatePicker.param.size.objects], pn.Spacer(height=425)
)
```
