...
- **`visual_size`** (str, default="medium"): Controls the visual size/density of the multi-choice control. One of "small", "medium", or "large".
...
Customize the visual size/density of the `MultiChoice` widget using the `visual_size` parameter:


```python
pn.FlexBox(*[pmui.MultiChoice(
    label=visual_size, value=['Apple', 'Pear'], options=['Apple', 'Banana', 'Pear', 'Strawberry'], visual_size=visual_size
) for visual_size in pmui.MultiChoice.param.visual_size.objects])
```
