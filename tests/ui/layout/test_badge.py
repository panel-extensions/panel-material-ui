...
def test_badge_basic(page):
    widget = Badge(IconButton(icon="mail"), badge_content=4, color="primary")
...
def test_badge_color(page):
    widget = Badge(IconButton(icon="mail"), badge_content=4, color="secondary")
    serve_component(page, widget)
...
def test_badge_max(page):
    widget = Badge(IconButton(icon="mail"), badge_content=100, max=99)
    serve_component(page, widget)
...
def test_badge_show_zero(page):
    widget = Badge(IconButton(icon="mail"), badge_content=0, show_zero=True)
    serve_component(page, widget)
...
def test_badge_zero_hidden_by_default(page):
    widget = Badge(IconButton(icon="mail"), badge_content=0)
    serve_component(page, widget)
...
def test_badge_overlap_circular(page):
    widget = Badge(IconButton(icon="mail"), badge_content=4, overlap="circular")
    serve_component(page, widget)
