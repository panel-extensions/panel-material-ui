...
IS_INSTALLED = in_site_packages(BASE_PATH)

BUNDLE_PATH = DIST_PATH / "panel-material-ui.bundle.js"
...
class MaterialComponent(ReactComponent):
    ...
    def _setup_autoreload(self):
        # Skips ReactiveESM._setup_autoreload, which would start a watcher
        # per instance, but keeps Syncable's stylesheet watching.
        Syncable._setup_autoreload(self)
        if IS_INSTALLED or not import_available('watchfiles'):
            # The bundle shipped in an installed distribution never changes,
            # watching it is only useful for a development install.
            return
        elif not (config.autoreload or getattr(self, '_debug', False)):
            return
        # Every component renders from the same bundle, so instead of the
        # per-instance watcher ReactComponent sets up we subscribe to a
        # single shared one.
        get_bundle_watcher(BUNDLE_PATH).subscribe(self)
